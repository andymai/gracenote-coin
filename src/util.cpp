...
{
    // interpret -noGNC as -GNC=0 (and -noGNC=0 as -GNC=1) as long as -GNC not set
    if (name.find("-no") == 0)
    {
        std::string positive("-");
...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        //  interpret --GNC as -GNC (as long as both are not set)
        if (name.find("--") == 0)
        {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gcoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it)
    {
        ...
        if (mapSettingsRet.count(strKey) == 0)
        {
            ...
            // interpret noGNC=1 as GNC=0 (and noGNC=0 as GNC=1) as long as GNC not set)
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gcoin.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong gcoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("gcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
